...
## Disclaimers
* In some examples, a massive stack of exceptions are manually shoved into one try block. This isn't standard practice and you'd normally test them via separate try blocks, loops, or conditional logic. More than one error **can** occur within a single block of code, however, with those examples simulating that likelihood as a visual contrast to the examples that don't use them.
* In some examples, raising an **ExceptionGroup** does not, itself, cause an exception! Python, instead, treats the act of **raising** an **ExceptionGroup** as a special form of "legal tender" that it accepts as a request (and squirrels away as though it never happened) in **exchange** for unpacking the container and presenting you **only** with all of its sub-exceptions instead of halting execution immediately.
